...
```{r}
#pg = island_sf[island_sf$PLN_AREA_N == "PUNGGOL",]
lixels <- lixelize_lines(island_df, 
                         750, 
                         mindist = 375)
```
...
```{r}
#| eval: false
lines_sf <- st_cast(island_sf, "MULTILINESTRING")
lines_sf <- st_cast(lines_sf, "LINESTRING")
test_sf <- st_cast(listings_sf, "LINESTRING")
#pg <- st_cast(pg, "MULTILINESTRING")
#pg <- st_cast(pg, "LINESTRING")
#roads_in_singapore <- st_cast(roads_in_singapore, "LINESTRING")
```

```{r}
#| eval: false
densities <- nkde(island_df, 
                  events = test_sf,
                  w = rep(1,nrow(test_sf)),
                  samples = samples,
