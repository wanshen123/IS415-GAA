...
```{r}
test_join <- left_join(filtered_tainan, test, by = c("TOWNNAME", "VILLNAME"))
```
...
```{r}
checkMe <- colSums(is.na(dengue_st))
print(checkMe)
```

```{r}
dengue_st <- dengue_st[!(is.na(dengue_st$count)), ]
```
